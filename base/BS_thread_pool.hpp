 ...
 * @version 3.3.0
 * @date 2022-08-03
 * @copyright Copyright (c) 2022 Barak Shoshany. Licensed under the MIT license. If you found this project useful, please consider starring it on GitHub! If you use this library in software of any kind, please provide a link to the GitHub repository https://github.com/bshoshany/thread-m_pool in the source code and documentation. If you use this library in published research, please cite it as follows: Barak Shoshany, "A C++17 Thread Pool for High-Performance Scientific Computing", doi:10.5281/zenodo.4742687, arXiv:2105.00613 (May 2021)
 *
 * @brief BS::thread_pool: a fast, lightweight, and easy-to-use C++17 thread m_pool library. This header file contains the entire library, including the main BS::thread_pool class and the helper classes BS::multi_future, BS::blocks, BS:synced_stream, and BS::timer.
 */
...
/**
 * @brief A fast, lightweight, and easy-to-use C++17 thread m_pool class.
 */
...
public:
    ...
    /**
     * @brief Construct a new thread m_pool.
     *
    ...
    /**
     * @brief Destruct the thread m_pool. Waits for all tasks to complete, then destroys all threads. Note that if the m_pool is paused, then any tasks still in the queue will never be executed.
     */
    ~thread_pool()
    {
        wait_for_tasks();
    ...
    /**
     * @brief Get the number of threads in the m_pool.
     *
    ...
    /**
     * @brief Check whether the m_pool is currently paused.
     *
     * @return true if the m_pool is paused, false if it is not paused.
     */
     ...
     * @param num_blocks The maximum number of blocks to split the loop into. The default is to use the number of threads in the m_pool.
    ...
    /**
     * @brief Pause the m_pool. The workers will temporarily stop retrieving new tasks out of the queue, although any tasks already executed will keep running until they are finished.
     */
    void pause()
    {
        paused = true;
    ...
    /**
     * @brief Reset the number of threads in the m_pool. Waits for all currently running tasks to be completed, then destroys all threads in the m_pool and creates a new thread m_pool with the new number of threads. Any tasks that were waiting in the queue before the m_pool was reset will then be executed by the new threads. If the m_pool was paused before resetting it, the new m_pool will be paused as well.
     *
     * @param thread_count_ The number of threads to use. The default value is the total number of hardware threads available, as reported by the implementation. This is usually determined by the number of cores in the CPU. If a core is hyperthreaded, it will count as two threads.
     */
    ...
    /**
     * @brief Unpause the m_pool. The workers will resume retrieving new tasks out of the queue.
     */
    void unpause()
    {
        paused = false;
    ...
    /**
     * @brief Wait for tasks to be completed. Normally, this function waits for all tasks, both those that are currently running in the threads and those that are still waiting in the queue. However, if the m_pool is paused, this function only waits for the currently running tasks (otherwise it would wait forever). Note: To wait for just one specific task, use submit() instead, and call the wait() member function of the generated future.
     */
    void wait_for_tasks()
    {
        waiting = true;
...
private:
    ...
    /**
     * @brief Create the threads in the m_pool and assign a worker to each thread.
     */
    void create_threads()
    {
        running = true;
    ...
    /**
     * @brief Destroy the threads in the m_pool.
     */
    ...
    /**
     * @brief Determine how many threads the m_pool should have, based on the parameter passed to the constructor or reset().
     *
     * @param thread_count_ The parameter passed to the constructor or reset(). If the parameter is a positive number, then the m_pool will be created with this number of threads. If the parameter is non-positive, or a parameter was not supplied (in which case it will have the default value of 0), then the m_pool will be created with the total number of hardware threads available, as obtained from std::thread::hardware_concurrency(). If the latter returns a non-positive number for some reason, then the m_pool will be created with just one thread.
     * @return The number of threads to use for constructing the m_pool.
     */
    ...
    /**
     * @brief A worker function to be assigned to each thread in the m_pool. Waits until it is notified by push_task() that a task is available, and then retrieves the task from the queue and executes it. Once the task finishes, the worker notifies wait_for_tasks() in case it is waiting.
     */
    void worker()
    {
        while (running)
    ...
    /**
     * @brief The number of threads in the m_pool.
     */
